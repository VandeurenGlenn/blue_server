		...
		return Promise.all(
			Object.values(listingsInfo).map(async (asset) => {
				...
				if (blueAsset.github.repos.length > 0) {
					const urlParts = (blueAsset.github.repos[0] as string)
						.replace('https://', '')
						.split('/');
					...
					// @ts-ignore
					if (repos.length > 0)
						blueAsset.github.repos = repos.map(
							(repo: GithubProjectResponse) => {
								return {
									name: repo.name,
					...
					for (const repo of repos) {
						...
						if (
							...
							promises.push(
								this.gitHub.getRepositoryCodeFrequency(owner, name)
							);
						} else {
