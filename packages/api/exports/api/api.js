...
export default class ServerApi {
    ...
    constructor(keys) {
        if (!keys)
            throw new Error('no api keys found (did you setup env?)');
     ...
     */
    async top100(limit = 100) {
        const listings = await this.coinMarketCap.getLatestListings(limit);
        ...
        return Promise.all(Object.values(listingsInfo).map(async (asset) => {
            const blueAsset = {
                id: asset.id,
            ...
            if (blueAsset.github.repos.length > 0) {
                ...
                // @ts-ignore
                if (repos.length > 0)
                    blueAsset.github.repos = repos;
                let promises = [];
                for (const repo of repos) {
                    // repo had activity within 7 days, so we try to get it's stats
                    if (new Date(repo.pushed_at).getTime() + SEVEN_DAYS_AGO >=
                        new Date().getTime()) {
                ...
                blueAsset.github.activity = promises.reduce((previous, current) => {
